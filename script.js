...
const box = document.querySelector('.hero .description-overlay');
const play = document.querySelector('.hero .description-overlay i');
...
function updateOverflowStatus() {
  if (checkOverflow(box)) {
    play.classList.remove('no-overflow');
    play.offsetWidth; // This forces a reflow so the animation restarts
    play.classList.add('overflowing');
  } else {
    play.classList.remove('overflowing');
    play.offsetWidth; // This forces a reflow so the animation restarts
    play.classList.add('no-overflow');
  }
}
